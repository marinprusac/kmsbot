...
def categorize_by_n_of_targets(players: list[Player]) -> tuple[list[Player], list[Player], list[Player], list[Player]]:
	zero: list[Player] = []
	one: list[Player] = []
	two: list[Player] = []
	not_targeting: list[Player] = []

	for p in players:
		count = 0
		if p.mission is None:
			not_targeting.append(p)
		for p2 in players:
		...
		if count == 0:
			...
			two.append(p)
	return zero, one, two, not_targeting
